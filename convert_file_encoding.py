...
#　转换目录下文件的编码
def convert_files_encoding(dir_path, new_dir_path, source_encoding, target_encoding):
    ...
    for filename in filelist:  # 遍历所有文件
        ...
        if filename_extension != ".java":    # 只修改扩展名为 .java 的文件，避免读取图片等二进制文件而导致的出错
            continue
...
if __name__ == '__main__':
    ...
    print(source_encoding, "转换为", target_encoding)
    dir_path = input("输入源文件夹路径：")
    new_dir_path = input("输入新文件夹路径：") 

    convert_files_encoding(dir_path, new_dir_path, source_encoding, target_encoding)
